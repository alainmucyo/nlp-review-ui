<template>
  <div class="p-8">
    <h1 class="text-2xl font-bold mb-4">Stakeholders</h1>
    ...
    >
      <div class="bg-white rounded-lg p-6 w-1/3">
        <h2 class="text-xl font-bold mb-4">Merge Stakeholders</h2>
        <p class="mb-4">Enter the new name for the merged stakeholder:</p>
        <input
            v-model="newMergedName"
            type="text"
    ...
    <div class="flex items-start">
      <table class="w-2/3 bg-white border border-gray-300 rounded-lg shadow-md">
        ...
        <tr class="bg-gray-100">
          ...
          <th class="py-2 px-4 text-left">Affiliation(s)</th>
        ...
        >
          ...
          <td class="py-2 px-4">{{ segment.data.name }}</td>
          <td class="py-2 px-4">
            <!-- Affiliation(s) rendering -->
            <div v-if="segment.data.affiliations">
              <ul class="list-disc list-inside">
                <li v-for="affil in segment.data.affiliations" :key="affil">
                  {{ affil }}
                </li>
              </ul>
            </div>
            <div v-else>
              {{ segment.data.affiliation }}
            </div>
          </td>
    ...
    async confirmMerge() {
      try {
        ...
        const selectedSegmentsData = this.segments.filter((segment) =>
            this.selectedSegments.includes(segment.segment_id)
        );

        // Collect affiliations from selected segments
        const affiliations = [];

        selectedSegmentsData.forEach((segment) => {
          const personData = segment.data;
          if (personData.affiliation) {
            affiliations.push(personData.affiliation);
          }
          if (personData.affiliations && Array.isArray(personData.affiliations)) {
            affiliations.push(...personData.affiliations);
          }
        });

        const mergedData = {
          name: this.newMergedName,
